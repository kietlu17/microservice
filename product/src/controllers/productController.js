  ...
  async createOrder(req, res, next) {
    try {
      ...
      this.ordersMap.set(orderId, { 
        status: "pending", 
        products, 
        username: req.user.username // trong jwt claim không có username ?
      });
  
      await messageBroker.publishMessage(config.rabbitMQQueueOrder, {
        products,
        username: req.user.username,
        orderId, // include the order ID in the message to orders queue
      });

      messageBroker.consumeMessage(config.rabbitMQQueueProduct, (data) => {
